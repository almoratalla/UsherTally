...
const SeatMapPage: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "seatMaps"), (snapshot) => {
      ...
      setSections((prevSections) => {
        const sectionMap = new Map(prevSections.map((s) => [s.sectionId, s]));
        ...
        <div className="grid gap-2">
          {Array.from({ length: currentSeatMap.rows }, (_, row) => (
            <div key={row} className="flex">
              {Array.from({ length: currentSeatMap.columns }, (_, column) => {
