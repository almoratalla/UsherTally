...
export function UserAuthForm({ className, form, ...props }: UserAuthFormProps) {
  ...
  const { setActiveUser, checkAuth } = useActiveUser();
      ...
      if (withGoogle) {
        await signInWithGoogle()
          .then(async (userCredential) => {
            ...
            const user = await checkAuth(uuid);
            setActiveUser(user as Partial<iActiveUser>);
          })
          .catch((error) => {
        ...
        const userCredential = await signIn(data.email, `${encrypted}`);
