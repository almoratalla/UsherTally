    ...
    setSelectedSeats(
      activeProjectSections.map((section) => ({
        ...
        seats: getSelectedSeatIndices(
          section.layout ? JSON.parse(section.layout as unknown as string) : [],
        ),
      })),
    );
  ...
  return (
    <div className="relative mx-auto w-full overflow-x-auto">
      ...
      >
        ...
        JSON.parse(selectedSection.layout as unknown as string).length > 0
          ? flattenLayoutWithNulls(
              JSON.parse((selectedSection.layout as unknown as string) ?? []),
