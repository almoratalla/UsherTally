...
export const SeatSection: React.FC<SeatSectionProps> = ({ columnsCount }) => {
  ...
  useEffect(() => {
    setSelectedSeats(
      activeProjectSections.map((section) => ({
        id: `${section.id}`,
        seats: section.layout ? getSelectedSeatIndices(section.layout) : [],
      })),
    );
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (isSelecting && containerRef.current && selectionBox) {
      ...
      const newHeight = e.clientY - containerRect.top - selectionBox.yStart;
          ...
          if (
            ...
            if (existingSelection) {
              updateSelectedSeats(sectionId, (prevSeats) =>
                [...prevSeats, seatIndex]
                  .filter((seat) =>
                    prevSeats.includes(seatIndex) ? seat !== seatIndex : true,
                  )
                  .filter(
  ...
  return (
    <div className="relative mx-auto w-full overflow-x-auto">
      ...
      >
        ...
        {selectedSection?.layout && selectedSection.layout.length > 0
          ? flattenLayoutWithNulls(selectedSection.layout).map(
              (seat, seatIndex, array) => {
