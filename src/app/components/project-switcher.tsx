...
const acceptedPublicRoutes = ["/demo"];
...
export default function ProjectSwitcher({ className }: TeamSwitcherProps) {
  ...
  if (!user && !acceptedPublicRoutes.includes(pathname)) return null;
  return (
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            ...
            <span className="w-4/5 overflow-hidden flex flex-row">
        ...
        <PopoverContent className="w-fit p-0">
          <Command>
            <CommandList>
              ...
              {projectsByType
                ?.filter(
                  (p) => (user ? p : p.type !== "private"), // public projects are always visible
                )
                .filter((p) =>
                  user
                    ? p.projects.some(
                    ...
                    {group.projects
                      ?.filter((p) =>
                        p.type === "public"
                          ? true
                          : user?.email
                            ? p.members.find((m) => m.email === user?.email) ||
                              p.owner === user?.email
                            : true,
                      )
                      ?.map((project) => (
                        <CommandItem
                          ...
                          className="text-sm w-[100px] sm:w-full"
                        >
                          <Avatar className="mr-2 h-5 w-5">
                            <AvatarImage
