...
export const useCounters = () => {
    ...
    const updateTrackUpdates = async (
        ...
        if (docSnap.exists()) {
            ...
            const colRef = collection(db, "trackUpdates");
            await setDoc(docRef, {
                day: dayNumber,
        ...
        console.log(
            `Updated trackUpdates for day ${dayNumber}, project ${projectName}, sections ${sectionIds}`
        );
    };
    ...
    const getTotalEditsForDay = (): number => {
        ...
        for (const project in counts) {
            if (counts.hasOwnProperty(project)) {
                const sectionCounts = counts[project];
