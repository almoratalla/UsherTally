...
export const useCounters = () => {
    ...
    const updateTrackUpdates = async (
        ...
        if (docSnap.exists()) {
            ...
            sectionIds.forEach((sectionId) => {
                updatedCounts[projectName][sectionId] = updatedCounts[
                    projectName
                ][sectionId]
    ...
    const { data: sectionsData, isLoading: isSectionsLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            const sectionsData: SectionStore[] = snapshot.docs
                .map((doc) => ({
                    id: doc.data().id,
    ...
    const { data: trackUpdatesData, isLoading: isTrackUpdatesLoading } =
        useQuery({
    ...
    const { mutate: mutateSectionCount } = useMutation({
        ...
        mutationFn: async (data: Section[]) => {
            ...
            const updatedSections = data
                ...
                .filter(Boolean) as unknown as Section[];
                        ...
                        body: JSON.stringify(
                            latestSectionCountUpdateRef.current.map(
                                (section) => ({
                                    ...
                                    layout: JSON.stringify(section.layout),
                                })
                            )
                        ),
                    });
    ...
    const { mutate: mutateSectionName } = useMutation({
        ...
        mutationFn: async (data: { id: number; name: string }) => {
            const section = sections.find(
                (s) =>
                    s.id === data.id &&
                    s.projectName === activeProject?.projectName
            );
            setSections((prevSections) => {
                if (section) {
                    return [
    ...
    const { mutate: mutateSectionsDelete } = useMutation({
        ...
        mutationFn: async (idToDelete: number) => {
            ...
            setSections((prevSections) =>
                prevSections.filter(
                    (section) =>
                        !sectionsToDeleteRef.current.includes(section.id)
                )
            );
            // Clear previous timer if it exists
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(
            ...
            (snapshot) => {
                const updatedSections: SectionStore[] = snapshot.docs
                    .map((doc) => ({
                        id: doc.data().id,
                        count: doc.data().count,
                        name: doc.data().name,
    ...
    useEffect(() => {
        ...
        channel.bind(
            "count-updated",
            (data: { id: number; count: number; layout: number[][] }) => {
                setSections((prevSections) =>
    ...
    const activeProjectSections = useMemo(
        ...
        [activeProject?.projectName, sections]
    );
    ...
    const totalEditsForLastFiveDays = useMemo(() => {
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            if (dayToCheck <= 0) {
                ...
                dayToCheck =
                    dayToCheck % daysInCurrentMonth || daysInCurrentMonth;
            }
                        ...
                        if (projectCounts) {
                            ...
                            for (const sectionId in projectCounts) {
                                if (projectCounts.hasOwnProperty(sectionId)) {
                                    totalEditsPerDay[day] +=
                                        projectCounts[sectionId];
                                }
                            }
                        }
                    }
